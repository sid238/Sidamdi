...
AMDI({ cmd: ["alive", "hi", "online"], desc: Lang.AliveDesc, type: "primary", react: "💃🏻" }, (async (amdiWA) => {
    ...
    if (ALIVE_MSG.input === 'default' || ALIVE_MSG.input == undefined) {
        const buttons = [
            {type: "url", displayText: "💃🏻 Official Website", url: 'https://github.com/sid238/Sidamdi'},
            {type: "url", displayText: "🎞️ singhaniya Tech YouTube Channel", url: 'https://www.youtube.com/channel/@singhaniyatech0744'},
...
AMDI({ cmd: "ping", desc: Lang.PingDesc, type: "primary", react: "🅿️" }, (async (amdiWA) => {
...
AMDI({ cmd: ["qaversion", "version"], desc: "Version check", cmdHideInMenu: true }, (async (amdiWA) => {
    ...
    return await reply(`*🧬 sid Amdi Version 🧬*\n\n` + '```Installed version``` : ' + version +'\n' + '\n```Check github``` : https://github.com/sid238/sidamdi/');
}));
